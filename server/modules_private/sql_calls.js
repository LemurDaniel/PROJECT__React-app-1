...
func = {};
    ...
    func.insert_user = async (con, user) => {
    ...
    func.get_password_hash = async (con, user) => {
        ...
        return new Promise( (resolve, reject) => {
            ...
            con.query(SQL_GET_HASH, [user.username], (err, res) => {
                ...
                if(err) reject(err);
                else if(res.length > 0) resolve( res[0].bcrypt.toString() );
    ...
    func.call_async = async statement  => { 
        return new Promise( (resolve, reject) => {
            func.pool.query(statement, (err, data) => {
                if(err) reject(err);
                else resolve(data);
            }) 
        });
    ...
    func.init_Database = async function (doodles_path) {
